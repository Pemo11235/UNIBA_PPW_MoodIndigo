class Plotter {
    constructor() {
        ...
        this.w = 500 - this.margin[1] - this.margin[3]; // width
        this.h = 300 - this.margin[0] - this.margin[2]; // height
        ...
        this.graph.append("text")
            ...
            .style("font-size", "100%")
            .text("Inclinazione della bocca vs Tempo");
        ...
        this.graph.append("text")
            ...
            .text("Inclinazione della bocca (Gradi)");
    ...
    change_graph(graphType) {
        ...
        switch (graphType) {
            case true:
                this.graph.selectAll(".yLabel").text("Rapporto Larghezza Bocca");
                this.graph.selectAll(".Title").text("Rapporto Larghezza Bocca vs Tempo");
                this.y.domain([0, 1])
                this.graph.select(".y.axis").call(this.yAxisLeft)
                break;
            case false:
                this.graph.selectAll(".yLabel").text("Inclinazione della bocca (Gradi)");
                this.graph.selectAll(".Title").text("Inclinazione della bocca vs Tempo");
                this.y.domain([-10, 10])
                this.graph.select(".y.axis").call(this.yAxisLeft)
                break;
            default:
...
(function() {
    ...
    message_text.innerHTML = "Caricamento detector";
    ...
    var onVidFileChosen = function(event) {
        ...
        if (canPlay === '') {
            message_text.innerHTML = "Fallimento nell'elaborazione del video. Assicurati sia mp4.";
        } else {
            message_text.innerHTML = "Analisi/Riproduzione del file video. Il download delle statistiche sarà disponibile al termine del video";
    ...
    resize_video_elements = function() {
        canvas_width = 300;
    ...
    var nextFrame = function() {
        ...
        if (vidTimeStamp <= video.duration) {
            ...
            message_text.innerHTML = ((vidTimeStamp / video.duration) * 100).toFixed(2) + "% Completato";
            video.currentTime = vidTimeStamp;
        } else {
            // DONE!, next action
            message_text.innerHTML = "100% Completato";
            alert("Video Processato");
    ...
    var initializeDetector = function() {
        ...
        detector.addEventListener("onInitializeSuccess", function() {
            ...
            plot_graph.style.display = "inline"
            document.getElementById('video_control').style.display = 'block'
            document.getElementById('fileUp').style.display = 'block'
        });
        ...
        detector.addEventListener("onImageResultsSuccess", function(faces, image, timestamp) {
            ...
            if (faces.length > 0) {
                ...
                mouth_ratio_txt.innerHTML = "Rapporto  apertura  bocca: " + mouth_ratio_val;
                if (graph_data_type) {
                ...
                emotion_txt.innerHTML = "Massima emozione: " + highest_emotion[0] + " " + highest_emotion[1];
                plotFeaturePoints(face.featurePoints)
                console.log("volto rilevato a" + timestamp.toFixed(2))
    ...
    draw_angles = function(angles) {
        var c = document.getElementById("angle_canvas");
        var ctx = c.getContext("2d");
        angle_intensity = 2; //determines how much the needle should move
        ctx.clearRect(0, 0, 300, 100);
        ctx.fillText("Altezza", 50, 10);
        ctx.fillText("Ondeggiamento", 150, 10);
        ctx.fillText("Straorzata", 250, 10);
        ctx.beginPath();
        ctx.moveTo(50, 50);
    ...
    faceToMouthRatio = function(face) {
    ...
    faceToMouthSlant = function(face) {
        var r_lip_coord = face.featurePoints[20];
        var l_lip_coord = face.featurePoints[24];
    ...
    faceToHighestEmotion = function(face) {
    ...
    plotFeaturePoints = function(featurePoints) {
    ...
    create_and_download_zip = function(csvOutputString) {
        var zip = new JSZip();
    ...
    reset_btn.onclick = function() {
        ...
        message_text.innerHTML = "Riavvio del detector";
        initializeDetector();
